...
function create_catch_block(ctx) {
	...
	return {
		c() {
			span = element("span");
			t0 = text("Could not load pages. Ensure file exists at ");
			t1 = text(/*site_url*/ ctx[1]);
			t2 = text("/primo.json");
		},
		l(nodes) {
			span = claim_element(nodes, "SPAN", {});
			var span_nodes = children(span);
			t0 = claim_text(span_nodes, "Could not load pages. Ensure file exists at ");
			t1 = claim_text(span_nodes, /*site_url*/ ctx[1]);
			t2 = claim_text(span_nodes, "/primo.json");
		...
		p(ctx, dirty) {
			if (dirty & /*site_url*/ 2) set_data(t1, /*site_url*/ ctx[1]);
		},
		i: noop,
		o: noop,
		d(detaching) {
...
// (127:10) {#if item.image && item.image.url}
function create_if_block$3(ctx) {
	...
	return {
		...
		h() {
			attr(img, "data-key", img_data_key_value = "items[" + /*i*/ ctx[11] + "].image");
			if (!src_url_equal(img.src, img_src_value = /*item*/ ctx[9].image.url)) attr(img, "src", img_src_value);
			attr(img, "alt", img_alt_value = /*item*/ ctx[9].image.alt);
...
function create_each_block$1(ctx) {
	...
	return {
		...
		i(local) {
			if (!li_intro) {
				add_render_callback(() => {
					li_intro = create_in_transition(li, fade, { delay: /*i*/ ctx[11] * 100 });
					li_intro.start();
				});
			}
		},
...
// (105:18)        <span>loading pages</span>     {:then value}
...
function create_fragment$3(ctx) {
	...
	handle_promise(promise = /*posts*/ ctx[2], info);
...
function instance$3($$self, $$props, $$invalidate) {
	...
	$$self.$$set = $$props => {
		...
		if ('site_url' in $$props) $$invalidate(1, site_url = $$props.site_url);
	};

	$$self.$$.update = () => {
		if ($$self.$$.dirty & /*site_url*/ 2) {
			 (get_posts());
		}
	};

	return [heading, site_url, posts, favicon, image, title, description];
}
